...
public class Storage implements IStorage{
    private static final Logger logger= LoggerFactory.getLogger("IStorage");
        ...
        if(sources.length>1){
            for(int i=0;i<sources.length;i++){
                try {
                    ...
                    logger.error("init source failed",e);
                }
            }
        }else{
            try {
    ...
    @Override
    public StorageSource achiveStorageSource() {
        return sources[0];
    }

    @Override
    public StorageSource achiveStorageSource(int splitID) {
        return sources[splitID%sources.length];
    }

    @Override
    public StorageConnect achiveConnection() {
