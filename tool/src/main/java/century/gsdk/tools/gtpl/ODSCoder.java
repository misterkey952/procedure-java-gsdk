...
import century.gsdk.tools.str.XJSON;
...
public abstract class ODSCoder{
    ...
    Class2Object readAsClass2Object(Class clazz) throws Exception {
        ...
        if(class2Object==null){
            ...
            if(sheet==null){
                ToolLogger.GTPL.error("there is no sheet for class {} ",clazz.getName());
                throw new Exception("there is no sheet for class");
            }
            ...
            ToolLogger.GTPL.info("read a sheet {} to class {} success",sheet.getName_zh(),clazz.getSimpleName());
        }
        return class2Object;
    }
    ...
    Object record2Object(ODSRecord odsRecord,Class clazz) throws Exception {
        ...
        for(Field field:fields){
            if(field.getAnnotation(Template.class)==null){
                continue;
            }
            ...
            if(keyValue==null){
                ToolLogger.GTPL.error("the class {} field {} is not defined in file {} sheet {}",
                        clazz.getName(),
                        field.getName(),
                        odsRecord.getSheet().getOdsFile().getFilePath(),
                        odsRecord.getSheet().getName_zh());
            ...
            if(StringTool.supportType(field.getType())){
                try{
                    ...
                    ToolLogger.GTPL.error("there is error keyValue [{}] in file {} sheet {} record [{}]",
                            XJSON.toJson(keyValue),
                            odsRecord.getSheet().getOdsFile().getFilePath(),
                            odsRecord.getSheet().getName_zh(),
                            XJSON.toJson(odsRecord.getKvList()));
                    throw e;
                }
    ...
    void convertRecord(Object obj,ODSSheet sheet) throws Exception {
        ...
        ODSRecord record=new ODSRecord(sheet);
        sheet.addRecord(record);
        records.add(obj);
        List<Field> fields=TypeAssistant.getFieldByAnnotation(obj.getClass(),Template.class);
        for(Field field:fields){
            boolean accessible=field.isAccessible();
            field.setAccessible(true);
            if(StringTool.supportType(field.getType())){
    ...
    void genSheetItera(Class rootClass) throws Exception {
        List<Field> fields=TypeAssistant.getFieldByAnnotation(rootClass,Template.class);
        for(Field field:fields){
    ...
    ODSSheet genSheet(Class clazz) throws Exception{
        ...
        sheet=new ODSSheet(clazz.getName(),template.name(),this.getOdsFile());
